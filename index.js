...
const bathroomMessage = 'It is not the cleanest bathroom but it will suffice. There just so happens to be a shower.'
...
let states = {
  Normal: { canChangeTo: ["ON FIRE!!!", "Hungry", "Full"] },
  Hungry: { canChangeTo: ["ON FIRE!!!", "Full", "DEAD!"] },
  Full: { canChangeTo: ["ON FIRE!!!", "Normal"] },
...
function simplifyString(string) {
  ...
  for (wordIndex in stringArray1) {
    if (stringArray1[wordIndex]==='the' || stringArray1[wordIndex]==='in' || stringArray1[wordIndex]==='into') {
...
function respond(answer) {
  ...
  if (
    ...
    if (currentLocation==="Shower" && showerStatus===1) {
      ...
      console.log("It occurs to you that you are now Enlightened. Congratulations, you have won the game!")
    }
    if (currentState==="Normal" || currentState==="Hungry") {
      enterState("Full");
      console.log("Ahhh, that 'za was delicious. You are now full.")
      drop("pizza")
    }
    if (currentState==="Full") {
      console.log("You eat the pizza even though you're already full. The wife won't be happy about this.")
      drop("pizza")
    }
    if (currentState==="ON FIRE!!!") {
      console.log("Pizza would be nice, but not being on fire would be better.")
      turnTaken = 0;
    }
  } else if (
  ...
  } else if (simplifyString(answer)==="xyzzy" || simplifyString(answer)==="combust") {
    if (currentState==="ON FIRE!!!") {
      console.log('You are already ' + currentState)
    } else if (currentLocation==="Shower" && showerStatus===1) {
      console.log('You momentarily light on fire, but the shower immediately puts the fire out.')
    } else {
      console.log("Rock Legend Dave Mustaine rises from the gates of hell and sets you ablaze!!!...The wife won't be happy about this.");
      enterState("ON FIRE!!!");
    }
  } else if (
    ...
    if ((simplifyString(answer)==="read sign" || simplifyString(answer)==="look at sign") && currentLocation==="182 Main St") {
      console.log(
      ...
      let theifStatus = 1;
    } else if (simplifyString(answer)==="open door" || simplifyString(answer)==="enter foyer") {
      console.log("The door is locked. There is a keypad on the door handle.");
    } else if ((simplifyString(answer)==="steal anyway" || simplifyString(answer)==="take anyway") && theifStatus===1) {
      console.log("As punishment for your insolence, rock Legend Dave Mustaine rises from the gates of hell and sets you ablaze!!!");
      enterState("ON FIRE!!!");
    } else if (
    ...
    } else if (simplifyString(answer)==="enter code 12345" || simplifyString(answer)==="key 12345") {
      enterRoom("Foyer");
      console.log(
        "Success! The door opens. You enter the foyer and the door shuts behind you..."
      );
    } else if (simplifyString(answer)==="enter mr mikes" || simplifyString(answer)==="go mr mikes") {
      enterRoom("Mr. Mike's");
      console.log("You head into Mr. Mike's Pizza.");
      if (simplifyString(answer).includes("eat pizza") && inventory.excludes("pizza")) {
        console.log("You don't have any pizza to eat.")
        turnTaken = 0; 
      } 
    }
  } else if (currentLocation==="Foyer") {
    if (simplifyString(answer)==="take paper" || simplifyString(answer)==="take seven days") {
    ...
    if (simplifyString(answer)==="enter bathroom" || simplifyString(answer)==="go bathroom") {
      enterRoom("Bathroom");
      console.log("You enter the bathroom.")
    }
    if (simplifyString(answer)==="enter classroom" || simplifyString(answer)==="go classroom") {
      if (currentState==="ON FIRE!!!") {
        console.log("If you went into class while being on fire, you might scare your classmates. Not such a good idea.")
        turnTaken = 0;
      } else {
        enterRoom("Classroom");
    ...
    if (simplifyString(answer).includes("leave") || simplifyString(answer)==="go outside" || simplifyString(answer)==="go main st") {
      enterRoom("182 Main St")
      console.log("You head back out onto the street.")
    }
  } else if (currentLocation==="Bathroom") {
    if (simplifyString(answer)==="enter shower" || simplifyString(answer)==="go shower") {
      enterRoom("Shower");
    ...
    if ((simplifyString(answer)==="use toilet" || simplifyString(answer)==="poop" || simplifyString(answer)==="pee") && bladderStatus===1) {
      console.log("Ahhh....You relieve yourself.")
      bladderStatus = 0;
    }
    if ((simplifyString(answer)==="use toilet" || simplifyString(answer)==="poop" || simplifyString(answer)==="pee") && bladderStatus===0) {
      console.log("You already relieved yourself.")
      turnTaken = 0;
    }
    if (simplifyString(answer)==="wash hands") {
      console.log("You wash your hands. They're nice and clean now.")
    }
    if (simplifyString(answer).includes("leave") || simplifyString(answer)==="go foyer" || simplifyString(answer)==="enter foyer") {
      enterRoom("Foyer");
    ...
    if (simplifyString(answer).includes("turn on") && showerStatus===1) {
      console.log("The shower is already on.")
      turnTaken = 0;
    }
    if (simplifyString(answer).includes("turn off") && showerStatus==1) {
      console.log("You turn the shower off. You are not cleaner, just wetter.")
      showerStatus = 0;
    }
    if (simplifyString(answer).includes("turn off") && showerStatus===0) {
      console.log("The shower is already off.")
      turnTaken = 0;
    }
    if (simplifyString(answer).includes("leave") || simplifyString(answer)==="go bathroom" || simplifyString(answer)==="enter bathroom") {
      enterRoom("Bathroom")
    ...
    if (simplifyString(answer)==="order pizza") {
      ...
      console.log("Mr. Mike himself makes you a pizza and sets it on the counter")
    }
    if (simplifyString(answer)==="take pizza" && inventory.includes("pizza")===false) {
    ...
    if (simplifyString(answer).includes("take pizza") && inventory.includes("pizza")) {
      console.log("You already have pizza.")
      turnTaken = 0;
    }
    if (simplifyString(answer).includes("leave") || simplifyString(answer)==="go outside" || simplifyString(answer)==="go main st" || simplifyString(answer)==="enter main st") {
      enterRoom("182 Main St")
    ...
    if (simplifyString(answer).includes("sit")) {
      if (currentState==="ON FIRE!!!") {
        console.log("If you sit down on this nice chair while you're on fire, you'll ruin the chair. You can't bring yourself to be that person.");
        turnTaken = 0;
      } else {
    ...
    if (simplifyString(answer)==="talk to josh") {
